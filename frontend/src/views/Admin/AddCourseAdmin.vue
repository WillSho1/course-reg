<template v-if="isAuthenticated">
  ...
  const scheduleObj = {};
  const dayPairs = scheduleStr.split(', ');
  dayPairs.forEach(pair => {
    const [day, timeRange] = pair.split(': ');
    scheduleObj[day.trim()] = { 'S': timeRange.trim() };
  });
  return scheduleObj;
}
  ...
  async function addsection() {
    ...
    course.value.Capacity = parseInt(course.value.Capacity, 10);
    course.value.Schedule = parseSchedule(course.value.Schedule);
